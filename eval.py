"""
Eval-Skript für die Streamlit-RAG-App (offline, ohne UI)

Funktionen:
- Lädt Fragen aus einer Datei (eine Frage pro Zeile)
- Verbindet sich mit Qdrant + lädt lokalen BM25-Index
- Führt Hybrid Retrieval (Vektor + BM25) und optionales Reranking aus
- (Optional) Ruft das LLM über eine OpenAI-kompatible API auf (OpenRouter o.ä.)
- Speichert Ergebnisse als CSV/Markdown in ./reports/

Nutzung:
  python eval.py --questions data/eval_questions.txt --llm \
    --topk_vec 50 --topk_bm25 50 --alpha 0.6 --k_final 8 --min_score 0.25 --max_chars 2400

ENV Variablen (wie App):
  QDRANT_URL, QDRANT_API_KEY
  OSS_API_BASE, OSS_API_KEY, OSS_MODEL  (nur wenn --llm gesetzt ist)

Hinweis: Dieses Skript importiert NICHT die Streamlit-App, damit es unabhängig vom UI läuft.
"""
from __future__ import annotations
import os, csv, argparse, json, hashlib, pickle
...
def get_qdrant_client():
    global _qdr
    if _qdr is None:
        _qdr = QdrantClient(url=os.getenv("QDRANT_URL", "http://localhost:6333"), api_key=os.getenv("QDRANT_API_KEY"))
    return _qdr
...
# ---------- Retrieval ----------
def hybrid_search(query: str, topk_vec: int, topk_bm25: int, alpha: float) -> List[Tuple[DocChunk,float]]:
    ...
    for r in res:
        p = r.payload
        vec_hits.append((DocChunk(p["text"], p["source"], int(p["chunk_id"]), {k:v for k,v in p.items() if k!="text"}), float(r.score)))
    # BM25
    if docs:
...
def rerank_snippets(query: str, candidates: List[Tuple[DocChunk,float]], k_final: int) -> List[Tuple[DocChunk,float]]:
    if not candidates: return []
    rr = get_reranker()
    pairs = [[query, d.text] for d,_ in candidates]
    scores = rr.predict(pairs)
    ranked = sorted([(candidates[i][0], float(scores[i])) for i in range(len(candidates))], key=lambda x: x[1], reverse=True)
    return ranked[:k_final]

def make_context(snips: List[Tuple[DocChunk,float]], max_chars: int, min_score: float, max_per_source: int = 2):
...
# ---------- LLM Call (optional) ----------
def call_oss_api(messages: List[Dict[str,str]], model: str, api_base: str, api_key: str) -> str:
...
def main():
    ...
    ap.add_argument('--questions', required=True, help='Pfad zur Fragen-Datei (eine Frage pro Zeile)')
    ap.add_argument('--llm', action='store_true', help='Antwort zusätzlich per LLM generieren')
    ...
    questions = [line.strip() for line in qfile.read_text(encoding='utf-8').splitlines() if line.strip() and not line.strip().startswith('#')]
    ...
    for q in questions:
        ...
        if args.llm and used:
            ...
            model = os.getenv('OSS_MODEL', 'meta-llama/llama-3-8b-instruct')
